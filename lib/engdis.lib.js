...
async getTestCodeDigit(code) {
  try {
    const runtimeUrl = `https://static.edappcdn.com/edprod01/edprod//Runtime/Lessons/${code}.js`;
    const { data } = await axios.get(runtimeUrl);

    // Validate the response data
    if (typeof data !== "string" || !data.includes("var lesson = ")) {
      throw new Error("Invalid response format: Expected JavaScript file with 'var lesson = '");
    }

    const lessonData = data.split("var lesson = ")[1].split(";")[0];
    const parsedData = JSON.parse(lessonData);

    // Validate the parsed data
    if (!parsedData.steps || !Array.isArray(parsedData.steps)) {
      throw new Error("Invalid lesson data: 'steps' array not found");
    }

    const filteredSteps = parsedData.steps.filter(item => item.id === '3');

    if (filteredSteps.length === 0) {
      throw new Error("No step with id '3' found");
    }

    return filteredSteps[0];
  } catch (err) {
    console.error("[!] Error in getTestCodeDigit:", err.message);
    return {
      isSuccess: false,
    };
  }
